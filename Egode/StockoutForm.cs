		...
		private class SfBillNewPrinter : SfBillPrinter
		{
			...
			*/
			...
			protected override void OnPrintPage(object sender, PrintPageEventArgs e)
			{
				...
				e.Graphics.DrawString("顺丰特惠", new Font("黑体", 12f), b, new PointF(CalcInch(77f), CalcInch(10f)));
...
//				</Request>";
		...
		private void StockoutForm_Load(object sender, EventArgs e)
		{
			...
			if (null == distributor)
			{
				txtSenderName.Text = ShopProfile.Current.DisplayNameOnBill;// "德 国 e 购";
		...
		{
			if (txtProvince.Text.Trim().StartsWith("上海"))
				return true;
			if (txtProvince.Text.Trim().StartsWith("浙江"))
				return true;
			if (txtProvince.Text.Trim().StartsWith("江苏"))
				return true;
			if (txtProvince.Text.Trim().StartsWith("安徽"))
				return true;
			return false;
		}
		...
		private void btnPrint_Click(object sender, EventArgs e)
		{
			...
			if (txtSenderName.Text.Equals("德 国 e 购"))
		...
		private void btnGo_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				// 不再按订单商品更新库存, 而是按界面上的商品列表更新库存.
				// 通常情况下, 界面上的商品列表就是订单商品. 如在界面上手动修改过商品信息, 则总是按修改后的商品信息更新库存.
				// 即, 自动更新库存总是和打印结果保持一致, 而非和订单保持一致.
				//foreach (SoldProductInfo spi in _soldProductInfos)
				//{
				//    ids += spi.Id + ",";
