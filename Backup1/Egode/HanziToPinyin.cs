...
/// <summary>
/// Summary description for HanziToPinyin
/// </summary>
...
{ -20319, -20317, -20304, -20295, -20292, -20283, -20265, -20257, -20242, 
 ...
 { "A", "Ai", "An", "Ang", "Ao", "Ba", "Bai", "Ban", "Bang", "Bao", "Bei", 
	...
	//private static string[] _unknownHz = new string[]{
	//    "÷ÿ«Ï,ChongQing",
	//    "„…––,MinHang",
	//    "„…,Min", "Á˚,Kun", "ÁÊ,Ke", "¬¶,Lou", "‹Ω,Fu", "Ô…,E", "Ïı,Zhen", "ˆŒ,Xin", "ÏË,Hu", "åu,Cai", 
	//    "Ÿª,Qian", "·È,Qu", "›∑,Xin", "›º,Jing", "Íª,Hao", "Á¯,Ying", "‰¡,Song", "Ë§,Yu", "„˛,Jing", "‰‡,Li", 
	//    "ˆ©,Wen", "Á˘,Qi", "‹∞,Xin", "Ë¥,Lu", "Ê¬,Yuan", "Íÿ,Xi", "ÙÀ,Xi", "Ó£,Rui", "Ü¥,Zhe", "‹«,Rui",
	//    "‚˘,Yi", "‰µ,Lai", "›Á,Jia", "⁄S,Yun", "˜»,Mei", "Ë™,Jin", "‰∫,Bang", "ˆ¡,Juan", "Ëµ,Bi", "€¥,Yin",
	//    "Ë∫,Tao", "ÊØ,Jiao", "ËÛ,Fan", "Ïø,Wei", "ÁÙ,Xi", "≠Z,Jing", "ËØ,Xuan", "Ê√,Ting", "€°,Di", "⁄˘,Wu",
	//    "Ú‘,Jiao", "‰∞,Hu", "Â˚,Yan", "Â°,Sui", "ïF,Min", "ÿÆ,Yu", "‹§,Dai" ,"ÿ∑,Yin", "‰•,Zhen", "Êº,Jie",
	//    "„—,Chang", "Êƒ,Wu", "Á„,Fen", "ÓÂ,Shuo", "Ó°,Sui", "Êµ,Xian", "·‘,Song", "ÍÃ,Yan", "Ë°,Chen", "ﬁ±,Wei",
	//    "€⁄,Zhen","Á‡,Ding", "Ô£,Zheng", "ÑÕ,Qing", "Á‚,Wei", "‰Ï,Xiao", "û›,Zhao", "Á˜,Qi", "‰Ú,Lian", "´h,Yue",
	//    "Ï”,Xuan", "¨u,Xuan", "›Ì,Bei", "Ê∫,Jing", "Ê©,Shan", "∫Ë,Hong", "Á‰,Wen", "È™,Nan", "Ïœ,Yu", "Í≈,Yu", 
	//    "È»,Jin", "Á±,Ling", "Ÿ‹,She", "‹ø,Yun", "⁄˜,Kuang", "©,Hao", "ˆ¶,Liang", "Ò˚,Tan", "·™,Qi", "ﬁ»,Yi",
	//    "ï◊,Zhao", "Íø,Xin", "Û„,Xiao", "ÏÕ,Yan", "Ê≠,Shu", "Ï≥,Fei", "˜Î,Lin", "‹∆,Zhi", "‹Á,Qian", "ÁÂ,Jue", "‰®,Zhu",
	//    "„∆,Yan", "Ë¶,Xia", "ÍÕ,Hui", "ÈÛ,Yi", "àê,Yao", "Ê´,Ya", "È,Ke", "ÿπ,Yu", "®,Jiao", "ŸÆ,Jiao", "Î¯,Teng",
	//    "Ò“,Zhu", "È≈,Rong", "Ê∑,Di", "‹”,Yi", "›Ê,Xuan", "·Ø,Cen", "ˆ≠,Fei", "Ó»,Zhao", "Í∞,Zang"};
	private static List<string> _unknownHz;
	private static List<string> UnknownHz
	{
		get
		{
			if (null == _unknownHz || _unknownHz.Count <= 0)
			{
				_unknownHz = new List<string>();
				string path = Path.Combine(Path.GetDirectoryName(Application.ExecutablePath), "hanzi.txt");
				if (File.Exists(path))
				{
					StreamReader reader = new StreamReader(path);
					while (!reader.EndOfStream)
						_unknownHz.Add(reader.ReadLine());
				}
			}
			return _unknownHz;
		}
	...
	//private static Dictionary<char, string> _hanzipinyin;
	
	//private static Dictionary<char, string> Hanzipinyin
	//{
	//    get
	//    {
	//        if (null == _hanzipinyin)
	//        {
	//            _hanzipinyin = new Dictionary<char,string>();
	//            ResourceManager rm = new ResourceManager("Egode.Properties.Resources", Assembly.GetExecutingAssembly());
	//            string s = rm.GetObject("hanzipinyin").ToString();
	//            //System.IO.StreamReader reader = new System.IO.StreamReader(stream);
				
	//            string[] hanzipinyinPairs = s.Split(new char[]{'\r', '\n'});
				
	//            foreach (string hanzipinyinPair in hanzipinyinPairs)
	//            {
	//                if (string.IsNullOrEmpty(hanzipinyinPair))
	//                    continue;
	//                char hanzi = hanzipinyinPair.Substring(0, 1).ToCharArray()[0];
	//                string pinyin = hanzipinyinPair.Substring(1, hanzipinyinPair.Length - 1);
	//                _hanzipinyin.Add(hanzi, pinyin.Split(new char[]{' '})[0]);
	//            }
	//        }
	...
	//// Writen by KK.
	//// Get pinyin from a text file of pinyin list directly.
	//public static string Convert2(string hzString)
	//{
	//    if (string.IsNullOrEmpty(hzString))// ‰»ÎŒ™ø’
	//        return null;
	...
	//    hzString = hzString.Replace("£¨", ",").Replace(str.ToString(), "").Replace(str.ToString(), "").Replace("£ª", "_").Replace("°£", "_").Replace("[", "").Replace("]", "").Replace("°æ", "").Replace("°ø", "");
	...
	//            else
	//            {
	//                pyString += Hanzipinyin[noWChar[j]];
	//            }
	//        }
	//        // ∑«÷–Œƒ◊÷∑˚      
	//        else
	...
	#region ∑Ω∑®µ˜”√_÷ª”–“ª∏ˆ≤Œ ˝

	public static string Convert(string hzString)
	{
		return Convert(hzString, 256);
	}
	...
	public static string Convert(string hzString, int maxLength)
	{
		...
		// ◊÷∑˚¥¶¿Ìby KK
		foreach (string s in UnknownHz)
		{
			try
			{
				string[] ss = s.Split(',');
				if (hzString.Contains(ss[0]))
					hzString = hzString.Replace(ss[0], ss[1]);
			}
			catch {}
		}
	...
	//{
	//    System.IO.FileStream fs = new System.IO.FileStream("e:\\hanzi.txt", System.IO.FileMode.Create);
	//    foreach(string s in _unknownHz)
	//    {
